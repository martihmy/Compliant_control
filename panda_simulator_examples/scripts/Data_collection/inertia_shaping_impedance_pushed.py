...
def impedance_control(rate,K_Pt,K_Po,K_m,K_d):
    ...
    for _iterations in range(max_num_it):
        ...
        h_c = np.dot(cartesian_inertia,alpha) + h_e + np.array(virtual_wrench.reshape((6,1)))
    ...
    np.save('is_pushed_virtual_forces.npy',virtual_wrench_list)
    np.save('is_pushed_sensor.npy',sensor_readings)
    np.save('is_pushed_sim_pose_error.npy',pose_error)
