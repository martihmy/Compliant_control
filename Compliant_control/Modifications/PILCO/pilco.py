...
class PILCO(gpflow.models.BayesianModel):
    ...
    def optimize_policy(self, maxiter=50, restarts=1):
        ...
        try:
            ...
            print('policy-optimization failed due to bad input (non-invertible matrix)')
        restarts -= 1

        best_parameter_values = [param.numpy() for param in self.trainable_parameters]
        best_reward = self.compute_reward()
        for restart in range(restarts):
            print('     - At restart: ',restart+1, ' out of: ',restarts+1)
    ...
    def compute_action(self, x_m, realtime=False):
        return self.controller.compute_action(x_m, tf.zeros([self.state_dim, self.state_dim], float_type), realtime=realtime)[0]
