...
def get_x_dot_numerically(x_hist,i,T):
...
def get_x(goal_ori):
    ...
    rel_ori = quatdiff_in_euler(np.asarray(robot.endpoint_pose()['orientation']), goal_ori)
...
def plot_result(v,p,p_d, delta_x, F_ext,F_d, z_dynamics,M,B,K, T):
...
if __name__ == "__main__":
    ...
    for i in range(max_num_it):
        ...
        x_dot = get_x_dot_numerically(x_history,i,T)
        v_history_num[:,i] = x_dot #positioning is important
        v_history[:,i] = get_x_dot()
    ...
    plot_result(v_history_num, p_history, p_d, delta_x_history, F_ext_history, F_d, z_dynamics_history,M,B,K, T)
