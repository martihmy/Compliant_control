...
data = np.array([[5.398,  2.822,  3.475],
                [5.575,  2.879,  3.17],
                [5.171,  2.905,  3.365],
                [5.014,  2.531,  3.27],
                [4.803,  2.38,  3.194],
                ...
                [4.333,  2.306,  5.72],
                [4.19,  2.09,  5.632]])
actual_rewards = np.array([24.3,29.68,33.67,39.33,36.76,40.64,22.24,30.48,31.58,23.96,40.68,4.52,0.85,25.15,40.75,40.72])
"""

#100Hz. SUBS=5, linear policy
data = np.array([[0.818,  0.187,  2.66],
                [0.276,  0.226,  3.164],
                [0.538,  0.178,  2.891],
                [0.651,  0.178,  2.964],
                [0.562,  0.259,  2.958],
                ...
                [0.298,  0.16,  1.605],
                [0.296,  0.144,  1.377]])
actual_rewards = np.array([81.64,78.43,61.94,81.62,74.78,81.26,72.74,80.25,78.56,78.17,80.52,81.58,74.47,80.91,79.73,81.65])
...
plt.suptitle('Trying to learn the transition dynamics of a dual environment', size =16)

plt.subplot(121)
plt.title('Variance in GP models')
plt.xlabel("Number of model optimizations")
xaxis = range(1,math.ceil(len(F))+1)
plt.xticks(xaxis)


plt.plot(list(range(1,len(F)+1)),F, label=r'$\sigma^2$ of force prediction')# (10 Hz)')
plt.plot(list(range(1,len(pos)+1)),pos, label=r'$\sigma^2$ of position prediction')# (10 Hz)')
plt.plot(list(range(1,len(vel)+1)),vel, label=r'$\sigma^2$ of velocity prediction')# (10 Hz)')
plt.legend()
...
xaxis = range(1,math.ceil(len(F))+1)
plt.xticks(xaxis)
plt.plot(range(1,len(actual_rewards)+1),actual_rewards/(20*5), label="10 Hz PILCO")
