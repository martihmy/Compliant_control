...
if __name__ == "__main__":
	...
	X1,Y1, _, _ = rollout(env=env, pilco=None, random=True, timesteps=T, SUBS=SUBS, render=False) # function imported from PILCO (EXAMPLES/UTILS)
	for i in range(1,5):
		X1_, Y1_,_,_ = rollout(env=env, pilco=None, random=True,  timesteps=T, SUBS=SUBS, render=False)
    	X1 = np.vstack((X1, X1_))
    	Y1 = np.vstack((Y1, Y1_))
	#env.close()
