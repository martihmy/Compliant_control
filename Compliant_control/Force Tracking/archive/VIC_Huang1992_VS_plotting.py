...
# Damping 
Bp = Kp/5 #/4
Bpz = Kpz/2 # #initial value (adaptive)
Bo = 300
...
#gamma_M = 12 
gamma_B = 0.001*5 #2    # The damping's rate of adaptivity (high value = slow changes)
gamma_K = 0.0005 *5#1    # The stiffness' rate of adaptivity (high value = slow changes)
#gamma[2,2] = gamma_M
gamma[8,8] = gamma_B
gamma[14,14] = gamma_K
...
def generate_desired_trajectory_tc(iterations,T,move_in_x=False):
    ...
    a[2,int(iterations*6/75):int(iterations*7/75)]= 1.25      
    if move_in_x:
...
def update_MBK_hat(lam,M,B,K):
    ...
    ensure_limits(1,500,B_hat)
    ensure_limits(1,1000,K_hat)
...
def plot_result(v_num, v,p,p_d, delta_x, F_ext,F_d, z_dynamics,M,B,K, T):
    ...
    plt.axhline(y=K[2][2], label = "initial stiffness (K_z)", color='b',linestyle='dashed')
    plt.xlabel("Real time [s]")
    plt.legend()
